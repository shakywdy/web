 ...
 * @LastEditTime: 2023-10-07 01:34:54
...
if (isset($_SESSION['user_id'])) {
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    // content
    $queryContent = "SELECT id,content,type,name,userid FROM message WHERE studentid = '$user_id'";
                ...
                while ($row = mysqli_fetch_assoc($resultContent)) {    
                  ...
                  echo '<ul id="message-list">
                      <li data-id="' . $messageid . '">
                          <div>
                              <span>Be from:' . $fdname . ' </span> 
                              ' .  $mscontent . '
                          </div>';
              //userid ==fd id
                  if ($row['type'] == 1) {
                    echo '<button onClick="addfd(' . $user_id . ', \'' . $fdid . '\');delthis(' . $messageid. ')" class="okbutton">
